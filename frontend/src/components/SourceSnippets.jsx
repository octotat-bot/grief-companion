...
export default function SourceSnippets({ examples }) {
  ...
  return (
    <div style={{ marginBottom: '1rem', border: '1px solid var(--ink-border)', borderRadius: '2px' }}>
      <button onClick={() => setOpen(v => !v)}
        ...
        <span style={{ color: 'var(--parch-ghost)', fontSize: '0.7rem' }}>{open ? '▲' : '▼'}</span>
      </button>
      {open && (
          ...
          {examples.map((ex, i) => (
            <div key={i} style={{ padding: '1rem', borderTop: '1px solid var(--ink-border)', background: 'var(--ink-soft)' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.5rem' }}>
                <span className="type-label" style={{ opacity: 0.5 }}>
              ...
              <p style={{ fontFamily: 'var(--font-display)', fontSize: '0.9rem', fontStyle: 'italic', color: 'var(--parch-ghost)', lineHeight: '1.7' }}>
                "{ex.text}"
              </p>
            </div>
          ))}
